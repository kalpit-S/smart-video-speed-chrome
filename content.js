...
function updateSpeedIndicator(speed) {
  let speedIndicator =
    document.getElementById("video-speed-indicator") || createSpeedIndicator();
...
function dynamicallyAdjust(target_wpm = 300, defaultSpeed = 1.0) {
  ...
  if (
    ...
    const transcriptIsOpen = !!document.querySelector(
      "ytd-transcript-segment-renderer"
    );
    ...
    if (!transcriptIsOpen) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          const transcriptIsOpenAfterClick = !!document.querySelector(
            "ytd-transcript-segment-renderer"
          );
...
function adjustVideoSpeedOnLoad() {
  ...
  chrome.storage.local.get(
    ["defaultSpeed", "targetWPM", "useWPMbyDefault"],
    (settings) => {
      const defaultSpeed = parseFloat(settings.defaultSpeed) || 1.0;
      const target_wpm = settings.targetWPM || 300;
      const useWPMbyDefault =
        settings.useWPMbyDefault !== undefined
          ? settings.useWPMbyDefault
          : true;
      if (useWPMbyDefault) {
