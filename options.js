    ...
    .then(
      (settings) => {
        ...
        document.getElementById("incrementAmount").value =
          settings.incrementAmount || 0.25;
        document.getElementById("targetWPM").value = settings.targetWPM || 300;
        document.getElementById("transcriptSeconds").value =
          settings.transcriptSeconds || 60;
        document.getElementById("useWPMbyDefault").checked =
          settings.useWPMbyDefault || false;
        document.getElementById("preset_speed_1").value =
          settings.preset_speed_1 || 1.0;
        document.getElementById("preset_speed_2").value =
          settings.preset_speed_2 || 2.0;
        document.getElementById("preset_speed_3").value =
          settings.preset_speed_3 || 3.0;
      },
      (error) => {
...
function saveSettings(event) {
  ...
  const transcriptSeconds = parseFloat(
    document.getElementById("transcriptSeconds").value
  );
  const useWPMbyDefault = document.getElementById("useWPMbyDefault").checked;
  const preset_speed_1 = parseFloat(
    document.getElementById("preset_speed_1").value
  );
  const preset_speed_2 = parseFloat(
    document.getElementById("preset_speed_2").value
  );
  const preset_speed_3 = parseFloat(
    document.getElementById("preset_speed_3").value
  );
